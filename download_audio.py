...
def download_split_by_chapters(url : str, chapters_to_exclude : list[str] = [], split : bool = True) -> None :
    """
    Download the audio of separated chapters (only kept chapters) from a video.
    ...
    if split == False and len(chapters_to_exclude) > 0:
        raise ValueError('If no split will be performed, no chapters to exclude should be provided.')
    ...
    with YoutubeDL(ydl_opts) as ydl:
        ...
        if split == False: # If we don't want to split the audio, we just stop after downloading it.
            return
        ...
        if all_chapters is None: # If we want to split but the audio has no chapters, we just stop after downloading it.
            return
        
        chapters_to_keep = [chap for chap in all_chapters if chap['title'] not in chapters_to_exclude] # The structure of all_chapters is list[dict[str, Any]
        ...
        for chap in chapters_to_keep:
            ...
            print(f'{title, start_time, duration, end_time}')
